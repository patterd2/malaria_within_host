...
for n = 1:nx-1 % evolving on the time since infection time scale
    ...
    %fprintf('%i \n',n);
    B(n+1) = (1/h*B(n) + P.lambda)./(1/h + P.lambda/P.K + P.p*M(n)+P.mu);
    M(n+1) = ((1-P.c)*P.p*P.beta*h*sum(gamma_fun(h*x).*I(n,:)'))/(1/h + P.muM) + ((1/h)/(1/h + P.muM)).*M(n);
    % BC for I
    I(n+1,1) = (1-P.c)*P.p*B(n+1)*M(n+1);
    % evolve I
    I(n+1,2:end) = (1/h)*I(n,1:end-1)./(1/h + P.mu + gamma_fun(h*x(2:end)') + P.sigma.*(1-exp(-P.theta*A(n))) );
    % BC for IG
    IG(n+1,1) = P.c*P.p*P.beta*h*sum(gamma_fun(h*x).*I(n,:)');
    % evolve IG
