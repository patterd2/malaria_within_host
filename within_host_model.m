function [B, M, I, IG, G, A] = within_host_model(h, t0, X_max, tau_max, B0, M0, I0, IG0, G0, A0)
...
for n = 1:nx-1 % evolving on the time since infection time scale
    ...
    M(n+1) = (M(n)/h + h*P.beta*sum(gamma_fun(tau,h).*I(n,:)'))./( 1/h + P.muM +P.p*B(n));
    % BC for I
    I(n+1,1) = (1-P.c)*P.p*B(n+1)*M(n+1);
    % evolve I
    I(n+1,2:end) = (I(n,1:end-1)/h)./(1/h + P.mu + gamma_fun(tau(2:end)',h) + P.sigma.*(1-exp(-P.theta*A(n))) );
    % BC for IG
