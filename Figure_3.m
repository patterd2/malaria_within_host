...
%% Plotting toggles
HEATMAP = 1; % Toggle for heatmap plotting, slow at high resolutions

%% numerical configuration
X_max = 700*24; % max time in days
...
for i = 1:2
    ...
    if i == 2 % turn off immunity for comparison
        P.sigma = 0;
    end
    ...
    % for a finer grid/higher quality figure
    ...
    hold on;
    if i == 1
        plot(invest,cum_inf1,'-','Color',[0.67578125 0.67578125 0.67578125],'LineWidth',4);
        psi = 1/105;
        cum_inf2 = simps(x(1:ac),betaHV(G_save(1:ac,:)).*repmat(exp(-psi*x(1:ac)/24),1,length(invest_vec)),1)/24;
        plot(invest,cum_inf2,':','Color',[0.67578125 0.67578125 0.67578125],'LineWidth',4);
        xlim([0.0 max(invest)]);
        ylim([0 300]);
        xticks([0 20 40 60]);
        xlim([0 60]);
        ...
        [~, B1] = max(cum_inf1);
        scatter(invest(B1),cum_inf1(B1),200,[0.67578125 0.67578125 0.67578125],'filled');
        [~, B2] = max(cum_inf2);
        scatter(invest(B2),cum_inf2(B2),200,[0.67578125 0.67578125 0.67578125],'filled');
        %[~, B] = max(cum_inf3);
    ...
    if i == 1
        figure(5);
        hold on;
        plot(invest,x(temp_rec)/24,'-','Color',[0.67578125 0.67578125 0.67578125],'LineWidth',4);
        yline(280,'--','Color',[0.67578125 0.67578125 0.67578125],'LineWidth',4,'Alpha',1);
        yline(1/psi,':','Color',[0.67578125 0.67578125 0.67578125],'LineWidth',4,'Alpha',1);
        ...
        box off;
        set(gca,'TickDir','out');
        scatter(invest(B1),temp_duration(B1),200,[0.67578125 0.67578125 0.67578125],'filled');
        legend('immune feedback','no immunity, no recovery',...
            'no immunity, constant recovery rate','location','northeast');
        %legend('boxoff')
        LimitsX = xlim; LimitsY = ylim;
